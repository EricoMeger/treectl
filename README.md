...
**Pré-split top-down**: essa decisão nasceu de um bug. Na primeira versão, a inserção descia recursivamente até a folha, inseria a chave e, se o nó estourasse o limite, dividia e devolvia pra cima qual chave deveria subir e qual nó novo tinha sido criado, o classico da btree. O problema é que os nós ficam guardados num `std::vector`, e o `std::vector` realoca toda a sua memória (movendo tudo pra um novo endereço) sempre que cresce além da capacidade atual. No meio da recursão a gente segurava uma referência pro nó pai, descia, criava um nó novo, o que podia disparar essa realocação, e na volta, ao tentar escrever a chave promovida nessa referência, ela já apontava pra um endereço que não existia mais dando segfault. Pesquisando uma forma melhor de estruturar isso, encontramos a abordagem de pré-split top-down do CLRS (Cormen): ao invés de dividir e propagar pra cima, a árvore desce do topo e divide cada nó cheio *antes* de entrar nele, eliminando esse vai-e-volta que dava pau.
